...
À ces cinq objets X++ s'ajoute la **solution Dataverse** créée en 3.7, qui sert de contexte à la connexion entre Visual Studio et l'environnement, et de conteneur à tout composant Power Platform qui viendrait compléter le dispositif.
...
**Contrôle 2, décisif.** Tentez la connexion depuis Visual Studio, décrite en 3.8. Si l'environnement apparaît dans la liste et propose des solutions, les outils de développement sont actifs. S'il n'apparaît pas, ou si aucune solution n'est proposée, c'est le premier soupçon.
...
### 3.1 Le poste dans l'architecture de l'environnement de développement unifié

Avant d'installer quoi que ce soit, il est utile de savoir précisément ce que l'on installe et pourquoi. Rappelons la définition posée au chapitre 2 : un **environnement de développement unifié** est un environnement Dynamics 365 Finance and Operations destiné au développement X++, mais **hébergé dans le Power Platform**. L'exécution est dans le cloud ; le poste, lui, est un atelier.

#### 3.1.1 Ce que le poste porte, et à quoi cela sert

| Élément installé | Rôle | Sans lui |
| :-- | :-- | :-- |
| **Visual Studio 2026 Professional** | Environnement de développement, éditeur, concepteurs d'objets, débogueur | Aucun développement X++ possible |
| Charge de travail `.NET desktop development` | Fournit MSBuild et l'infrastructure .NET sur laquelle reposent les outils | Les concepteurs et la compilation ne fonctionnent pas |
| Composant `Modeling SDK` | Infrastructure de modélisation des concepteurs Dynamics 365 | Les concepteurs de table et de formulaire ne s'ouvrent pas |
| Composant `DGML editor` | Visualisation des graphes de dépendances | Fonctionnalité de navigation en moins, non bloquant |
| `SQL Server Express LocalDB` | Héberge la base de références croisées qui indexe le code | La recherche et la navigation dans l'`Application Explorer` sont dégradées |
| Extension **`Power Platform Tools`** | Établit la connexion à Dataverse, télécharge les métadonnées, installe l'extension Finance and Operations | Impossible de se connecter à l'environnement |
| Extension **Finance and Operations** | Ajoute le menu `Dynamics 365`, les concepteurs X++, le déploiement et le débogage | Aucun outil X++ dans Visual Studio |
| Extension `Microsoft Reporting Services Projects` | Concepteur d'états | Impossible de développer un état |
| **Métadonnées de référence**, environ 24 Go | Définition complète de l'application standard, en lecture | Rien à étendre, rien à compiler contre |
| **Base de références croisées** | Index de navigation entre objets | Recherche lente ou inopérante |
| **Vos métadonnées personnalisées** | Vos modèles, source de vérité de votre code | Rien à versionner ni à déployer |

#### 3.1.2 Ce que le poste ne porte pas

Ces absences sont volontaires et structurantes. Les méconnaître conduit à chercher sur le poste ce qui n'y est pas.

| Absent du poste | Où cela se trouve | Conséquence pratique |
| :-- | :-- | :-- |
| Serveur d'application | Environnement cloud | Une compilation réussie ne fait rien fonctionner tant que le modèle n'est pas déployé |
| Base de données applicative | Environnement cloud | La synchronisation de schéma s'exécute sur le cloud, pas en local |
| Données de démonstration | Environnement cloud | Les tests fonctionnels se font dans le navigateur, sur l'application |
| Moteur d'exécution du X++ | Environnement cloud | Le débogueur s'attache à distance, il ne s'exécute pas localement |

#### 3.1.3 Vue d'ensemble de la phase

| Étape | Section | Durée indicative |
| :-- | :-- | :-- |
| Préparer le poste | 3.2 | 20 min |
| Obtenir Visual Studio 2026 | 3.3 | 10 min |
| Installer charges de travail et composants | 3.4 | 30 à 60 min |
| Installer les deux extensions de la place de marché | 3.5 | 15 min |
| Régler les options de l'extension | 3.6 | 10 min |
| Créer la solution et l'éditeur | 3.7 | 10 min |
| Se connecter et télécharger les métadonnées | 3.8 | 1 à 2 h |
| Vérifier la configuration des métadonnées | 3.9 | 5 min |
| Fiabiliser le poste après installation | 3.10 | 15 min |
| Contrôler que le poste est opérationnel | 3.11 | 10 min |

**L'ordre compte.** Chaque étape dépend de la précédente. En particulier, l'extension Finance and Operations ne s'installe qu'à la première connexion réussie à l'environnement, en 3.8 : elle ne se télécharge pas depuis la place de marché.

### 3.2 Préparer le poste avant toute installation

Cette section réduit sensiblement le nombre d'incidents rencontrés ensuite. Elle prend vingt minutes et en fait gagner plusieurs.

#### 3.2.1 Contrôler la configuration matérielle et logicielle

| Point de contrôle | Exigence | Comment vérifier |
| :-- | :-- | :-- |
| Système d'exploitation | Windows 10 64 bits ou Windows 11 | `winver` dans la boîte d'exécution |
| Mémoire vive | 16 Go minimum, 32 Go recommandés | Gestionnaire des tâches, onglet `Performance` |
| Espace disque libre | 60 Go minimum, 100 Go recommandés, de préférence sur SSD | Explorateur de fichiers |
| Droits | Administrateur local | Panneau de configuration, comptes d'utilisateurs |
| Connexion Internet | 20 Mbit/s minimum | Le téléchargement des métadonnées porte sur environ 24 Go |

**Sur la mémoire.** Seize gigaoctets suffisent pour le composant de ce guide. Un modèle volumineux, ou plusieurs instances de Visual Studio ouvertes, saturent rapidement cette configuration. La compilation X++ est le poste de consommation principal.

#### 3.2.2 Activer la prise en charge des chemins longs

Les chemins des métadonnées X++ sont profonds. Combinés à un dossier de dépôt lui-même profond, ils dépassent la limite historique de Windows et produisent des erreurs de compilation dont le message n'évoque jamais la longueur du chemin.

**Étape 1.** Ouvrez une invite de commandes **en tant qu'administrateur**.
...
**Étape 3.** Redémarrez le poste pour que le réglage prenne effet.

**Précaution complémentaire.** Indépendamment de ce réglage, gardez des chemins courts : `C:\D365` pour les métadonnées et `C:\D365\Repos` pour les dépôts. C'est la mesure la plus efficace.

#### 3.2.3 Préparer l'arborescence de travail

Créez dès maintenant les dossiers qui seront utilisés dans les chapitres suivants.

| Dossier | Usage | Chapitre où il sert |
| :-- | :-- | :-- |
| `C:\D365` | Racine de travail | 3 |
| `C:\D365\Repos` | Dépôts Git clonés | 4 |
| `C:\D365\Assets` | Emplacement de repli pour les métadonnées de référence, si vous les déplacez | 3 |

**Note.** Par défaut, les métadonnées de référence sont téléchargées dans le profil utilisateur, sous `AppData`. Ce comportement convient dans la plupart des cas. Ne le modifiez que si votre disque système est trop petit.

#### 3.2.4 Configurer les exclusions de l'antivirus

C'est le réglage le plus rentable de cette section, et le plus souvent omis.

Les dossiers de métadonnées contiennent des dizaines de milliers de petits fichiers XML. Une analyse antivirus en temps réel les inspecte un par un à chaque compilation, ce qui **multiplie les temps de génération**, parfois par trois ou quatre. Le symptôme est une compilation anormalement lente sans erreur apparente.

**Dossiers à exclure de l'analyse en temps réel :**

| Chemin | Contenu |
| :-- | :-- |
| `C:\Users\<Utilisateur>\AppData\Local\Microsoft\Dynamics365` | Métadonnées de référence et assets téléchargés |
| `C:\D365` | Vos métadonnées, vos dépôts et vos projets |
| Le dossier d'installation de Visual Studio | Binaires de l'environnement de développement |

**Étape 1.** Ouvrez `Sécurité Windows` > `Protection contre les virus et menaces` > `Gérer les paramètres` > `Ajouter ou supprimer des exclusions`.

**Étape 2.** Ajoutez chacun des dossiers ci-dessus comme exclusion de type dossier.

**Point de vigilance.** Une exclusion antivirus est une décision de sécurité. Sur un poste d'entreprise, elle relève de votre équipe sécurité et non de votre seule initiative. Présentez-la comme une exclusion ciblée sur des dossiers de développement, et non comme une désactivation de la protection.

#### 3.2.5 Vérifier les accès réseau

Si votre poste est derrière un proxy ou un pare-feu filtrant, plusieurs services doivent être joignables. Un blocage se manifeste par des échecs de connexion ou de téléchargement dont le message ne désigne jamais le proxy.

| Service | Domaine concerné | Étape qui échoue sans lui |
| :-- | :-- | :-- |
| Authentification Microsoft | `login.microsoftonline.com` | Toute connexion |
| Dataverse | `*.dynamics.com` | Connexion à l'environnement, 3.8 |
| Application Finance and Operations | `*.operations.dynamics.com` | Tests fonctionnels dans le navigateur |
| Place de marché Visual Studio | `marketplace.visualstudio.com` | Installation des extensions, 3.5 |
| Téléchargement des assets | Points de distribution Microsoft | Téléchargement des métadonnées, 3.8 |
| Azure DevOps | `dev.azure.com`, `*.pkgs.visualstudio.com` | Dépôt et flux d'artefacts, chapitres 4 et 8 |

**Test rapide.** Ouvrez successivement, dans le navigateur du poste, l'adresse Dataverse de votre environnement et l'adresse de l'application. Si les deux se chargent, l'essentiel du chemin réseau est ouvert.

#### 3.2.6 Checklist de la section

- [ ] Le poste satisfait les exigences matérielles.
- [ ] Vous disposez des droits d'administrateur local.
- [ ] La prise en charge des chemins longs est activée et le poste a redémarré.
- [ ] L'arborescence `C:\D365` est créée.
- [ ] Les exclusions antivirus sont posées, en accord avec votre équipe sécurité.
- [ ] Les adresses Dataverse et Finance and Operations se chargent dans le navigateur.

### 3.3 Obtenir Visual Studio 2026 Professional
...
### 3.4 Installer les charges de travail et les composants individuels
...
### 3.5 Installer les extensions requises

Deux extensions issues de la place de marché sont nécessaires, en plus de l'extension Finance and Operations qui sera installée automatiquement en 3.7.

#### 3.5.1 Microsoft Reporting Services Projects
...
### 3.6 Configurer les options Power Platform Tools
...
#### 3.6.1 Tableau de configuration recommandée
...
| `Enable auto setup for Dynamics 365 when using the Unified environment` (Activer la configuration automatique pour Dynamics 365 en environnement unifié) | **Activée** | Après le téléchargement des assets, extrait automatiquement les métadonnées de référence et crée la configuration de métadonnées décrite en 3.9. Désactivée, il faut débloquer les fichiers, décompresser une archive de 24 Go et renseigner la configuration à la main. Le gain de temps est de l'ordre de l'heure. |
...
#### 3.6.2 Lecture d'ensemble de ces réglages
...
#### 3.7.1 Ce qu'est une solution, et pourquoi elle compte
...
| `Publisher` (Éditeur) | `ARCHIA365`, créé en 3.7.2 | |
...
#### 3.7.4 Ce que contiendra cette solution
...
### 3.8 Se connecter à l'environnement et télécharger les métadonnées
...
**Étape 6.** Une solution vous est demandée. Sélectionnez **`ARC Revue de credit client`**, créée en 3.7.

**Point de vigilance.** **Ne choisissez jamais la solution `Default`** (Par défaut), pour les trois raisons exposées en 3.7.1 : préfixe aléatoire, impossibilité d'export propre, et mélange des composants de toutes provenances. Si votre solution n'apparaît pas dans la liste, c'est presque toujours qu'elle a été créée dans un autre environnement que celui auquel Visual Studio est connecté. Retournez sur `https://make.powerapps.com`, vérifiez le sélecteur d'environnement en haut à droite, et recréez la solution dans le bon environnement.
...
### 3.9 Vérifier la configuration des métadonnées

L'option `Auto setup` ayant été activée en 3.6, la configuration a normalement été créée automatiquement. Cette section vérifie qu'elle est correcte, et explique comment la créer à la main si nécessaire.
...
| `Application version to restore cross reference database from` (Version d'application source) | La version téléchargée en 3.8 |
...
### 3.10 Fiabiliser le poste après installation

L'installation est terminée, mais trois réglages complémentaires évitent des incidents récurrents.

#### 3.10.1 Compléter les exclusions antivirus

Les dossiers réellement utilisés ne sont connus qu'après l'installation. Complétez les exclusions posées en 3.2.4 avec les chemins effectifs.

**Étape 1.** Relevez le chemin exact des métadonnées de référence, visible dans `Configure Metadata` (Configurer les métadonnées), champ `Folders for reference metadata` (Dossiers des métadonnées de référence).

**Étape 2.** Relevez le chemin de la base de références croisées, généralement sous le profil utilisateur.

**Étape 3.** Ajoutez ces chemins aux exclusions, s'ils ne sont pas déjà couverts.

**Vérification du gain.** Lancez une génération de projet et notez sa durée. Après exclusion, une génération qui prenait plusieurs minutes tombe fréquemment sous la minute pour un petit modèle.

#### 3.10.2 Vérifier l'instance LocalDB

La base de références croisées vit dans une instance `SQL Server Express LocalDB`. Si elle n'a pas démarré, la navigation dans le code est dégradée et la configuration des métadonnées refuse de s'enregistrer.

**Étape 1.** Ouvrez une invite de commandes et exécutez :

```
sqllocaldb info
```

**Étape 2.** L'instance `MSSQLLocalDB` doit figurer dans la liste. Si elle est absente, créez-la :

```
sqllocaldb create MSSQLLocalDB -s
```

**Étape 3.** Vérifiez son état :
...
#### 3.10.3 Régler la sortie de Visual Studio pour le suivi des opérations

Les opérations longues, téléchargement des métadonnées, déploiement de modèle et synchronisation de base, écrivent dans une source de sortie dédiée. La connaître évite de croire qu'il ne se passe rien.

**Étape 1.** Ouvrez `View` (Affichage) > `Output` (Sortie).

**Étape 2.** Dans la liste déroulante `Show output from` (Afficher la sortie de), sélectionnez **`FinOps Cloud Runtime`**.

**Étape 3.** Épinglez cette fenêtre : elle sera votre principal instrument de suivi et de diagnostic pour tout le reste du guide.

### 3.11 Contrôler que le poste est opérationnel

Avant de passer au chapitre suivant, déroulez ces contrôles. Chacun isole une brique différente de la chaîne, ce qui permet de localiser un défaut plutôt que de constater un échec global.

| Contrôle | Comment procéder | Résultat attendu | Ce qu'un échec signifie |
| :-- | :-- | :-- | :-- |
| Visual Studio est activé | `Help` (Aide) > `About` (À propos) | Édition Professional, licence valide | Connexion Dev Essentials à refaire |
| Les extensions sont chargées | `Extensions` > `Manage Extensions` > `Installed` (Installées) | `Power Platform Tools` et `Microsoft Reporting Services Projects` présents et activés | Installation incomplète, ou Visual Studio non redémarré |
| L'extension Finance and Operations est installée | Barre de menus | Un menu **`Dynamics 365`** est présent | La connexion de 3.8 n'a pas abouti, ou l'installation n'a pas été lancée |
| La connexion à Dataverse est établie | `Tools` (Outils) > `Connect to Dataverse` | L'environnement est listé et propose des solutions | Voir l'avertissement d'authentification en 3.8 |
| Les métadonnées sont présentes | Explorateur de fichiers, dossier des assets | Environ 24 Go | Téléchargement incomplet, relancer `Download Dynamics 365 assets` |
| La configuration des métadonnées est valide | `Dynamics 365` > `Configure Metadata` | Une configuration existe et s'enregistre sans erreur | Chemins erronés, ou LocalDB indisponible |
| L'indexation du code fonctionne | `View` > `Application Explorer`, recherche de `CustTable` | La table est trouvée et son concepteur s'ouvre | Base de références croisées non générée |
| Les commandes de développement sont disponibles | Menu `Dynamics 365` | `Model Management`, `Configure Metadata`, `Deploy`, `Synchronize database` sont présents | Extension Finance and Operations partiellement installée |
| La commande des NuGets est disponible | Menu `Tools` | `Download Dynamics 365 FinOps NuGets for CI/CD` est présent | Option correspondante inactive, voir 3.6.1 |
| La sortie de suivi est accessible | `View` > `Output`, liste déroulante | La source `FinOps Cloud Runtime` existe | Aucune opération n'a encore été lancée, ce qui est normal à ce stade |

**Ce que ce contrôle ne couvre pas encore.** La chaîne complète, écrire du X++, le compiler, le déployer et l'exécuter, ne peut être validée qu'une fois un modèle et un projet créés. Cette validation de bout en bout est réalisée au chapitre 5, section 5.4, au moyen d'une classe exécutable minimale. **Ne passez pas au développement du composant sans avoir réussi ce test.**

### 3.12 Checklist de validation de la phase 1
...
- [ ] Les dix options `Power Platform Tools` sont réglées conformément au tableau 3.6.1.
...
### 5.4 Valider la chaîne complète par une classe exécutable

Vous disposez maintenant d'un poste installé, d'un environnement connecté, d'un modèle et d'un projet. Il reste à vérifier que l'ensemble fonctionne **de bout en bout**, avant d'investir du temps dans le composant du chapitre 6.

Le test consiste à écrire dix lignes de X++, à les compiler, à les déployer et à les exécuter sur l'environnement cloud. Il valide en une seule fois le compilateur local, la configuration des métadonnées, le canal de déploiement, la connexion à l'environnement et l'exécution distante.

**Rôle requis.** Administrateur local du poste pour la compilation. `System administrator` (Administrateur système) dans l'environnement pour le déploiement et l'exécution.

#### 5.4.1 Créer une classe exécutable
...
**Étape 2.** Dans la catégorie `Dynamics 365 Items` (Éléments Dynamics 365), sélectionnez **`Runnable Class`** (Classe exécutable).

**Étape 3.** Nommez-la `ArcHelloWorld` et cliquez sur `Add` (Ajouter).

**Différence avec une classe ordinaire.** Une classe exécutable est une classe dotée d'une méthode `main`, point d'entrée que la plateforme sait invoquer directement depuis une adresse. C'est l'équivalent X++ d'un programme lançable, et l'outil de diagnostic le plus simple dont vous disposiez.

**Étape 4.** Remplacez le contenu du fichier par le code suivant :

```xpp
/// <summary>
/// Classe de validation de la chaine de developpement.
/// Ecrit un message dans la fenetre d'information de l'application.
/// Auteur : ARCHIA365
/// </summary>
internal final class ArcHelloWorld
{
    public static void main(Args _args)
    {
        info('Chaine de developpement operationnelle, execution cote serveur.');
        info(strFmt('Date de session : %1', systemDateGet()));
        info(strFmt('Utilisateur courant : %1', curUserId()));
    }
}
```
...
| Élément | Rôle |
| :-- | :-- |
| `internal final class` | Visibilité limitée au modèle, classe non héritable. Convention pour une classe utilitaire |
| `public static void main(Args _args)` | Point d'entrée obligatoire d'une classe exécutable. La signature ne se modifie pas |
| `info(...)` | Écrit un message dans la barre d'information de l'application, visible par l'utilisateur |
| `strFmt('... %1', valeur)` | Compose une chaîne en substituant les jetons numérotés. Équivalent X++ du formatage de chaîne |
| `systemDateGet()` | Retourne la date de session de l'utilisateur |
| `curUserId()` | Retourne l'identifiant de l'utilisateur courant |

**Étape 5.** Enregistrez par `Ctrl+S`.

#### 5.4.2 Compiler

**Étape 1.** Faites un clic droit sur le projet, puis `Build` (Générer).

**Étape 2.** Ouvrez `View` (Affichage) > `Error List` (Liste d'erreurs).

**Étape 3.** Ouvrez également `View` (Affichage) > `Output` (Sortie), source `Build` (Génération), pour suivre le détail.

**Résultat attendu.** La génération se termine par un message de réussite, sans erreur ni avertissement bloquant.

**Ce que la compilation valide déjà.** Le compilateur X++ est installé et fonctionnel, les métadonnées de référence sont accessibles, votre modèle est correctement déclaré et ses packages de référence sont résolus. Une erreur à ce stade est locale : elle ne met en cause ni l'environnement, ni le réseau.

| Message d'erreur fréquent | Cause | Correction |
| :-- | :-- | :-- |
| Type ou méthode introuvable pour un objet standard | Packages de référence insuffisants | `Model Management` > `Update model parameters`, ajoutez le package manquant |
| Le projet ne contient aucun élément à compiler | La classe n'a pas été ajoutée au projet | Vérifiez sa présence dans le `Solution Explorer` |
| Génération anormalement lente, sans erreur | Analyse antivirus des métadonnées | Appliquez les exclusions de la section 3.10.1 |
| Erreur mentionnant un chemin | Chemin trop long | Voir la section 3.2.2 |

#### 5.4.3 Déployer sur l'environnement

La compilation a produit des binaires **sur votre poste**. Ils n'existent pas encore pour l'environnement cloud.

**Étape 1.** Faites un clic droit sur le projet, puis `Deploy model for project` (Déployer le modèle du projet). Selon la version, la commande peut se trouver sous `Extensions` > `Dynamics 365` > `Deploy` (Déployer).

**Étape 2.** La synchronisation de la base n'est **pas nécessaire ici** : cette classe ne modifie aucune structure de données. Si la boîte de dialogue le propose, vous pouvez décocher l'option.

**Étape 3.** Suivez la progression dans `View` (Affichage) > `Output` (Sortie), source **`FinOps Cloud Runtime`**, réglée en 3.10.3.

**Durée.** Comptez de cinq à vingt minutes pour un premier déploiement, moins ensuite.

**Ce que le déploiement valide.** La connexion à l'environnement, les droits de votre compte, la présence effective des outils de développement côté serveur, et le canal de transport entre le poste et le cloud. Un échec ici est le premier signe sérieux d'un problème d'environnement ou de droits.

| Symptôme | Cause probable | Correction |
| :-- | :-- | :-- |
| Le déploiement est refusé pour cause de droits | Rôle `System administrator` absent | Attribuez-le dans l'application, `System administration` > `Users` |
| Le déploiement échoue immédiatement | L'environnement n'a pas les outils de développement | Voir la section 1.7 |
| Le déploiement reste bloqué sans progression | Opération concurrente sur l'environnement | Vérifiez l'état de l'environnement dans le portail et attendez |
| Aucune sortie visible | Mauvaise source sélectionnée dans la fenêtre de sortie | Sélectionnez `FinOps Cloud Runtime` |

#### 5.4.4 Exécuter la classe

Deux voies mènent au même résultat.

**Voie 1, depuis Visual Studio.**

**Étape 1.** Dans le `Solution Explorer`, faites un clic droit sur le projet, puis `Properties` (Propriétés).

**Étape 2.** Dans le champ `Startup Object` (Objet de démarrage), sélectionnez `ArcHelloWorld`.

**Étape 3.** Appuyez sur `Ctrl+F5`, qui exécute sans débogage. Le navigateur s'ouvre sur l'environnement et exécute la classe.

**Voie 2, par l'adresse directe.** Utile quand vous voulez exécuter sans passer par Visual Studio, ou vérifier depuis un autre poste.

**Étape 1.** Composez l'adresse suivante, en substituant vos valeurs :

```
https://<votre-environnement>.sandbox.operations.dynamics.com/?mi=SysClassRunner&cls=ArcHelloWorld
```

**Étape 2.** Ouvrez-la dans le navigateur, connecté avec un compte disposant des droits.

**Résultat attendu.** L'application affiche une barre d'information contenant les trois messages :

```
Chaine de developpement operationnelle, execution cote serveur.
Date de session : 25/08/2026
Utilisateur courant : admin
```

**Ce que l'exécution valide.** Le code que vous avez écrit sur votre poste s'exécute réellement sur le serveur d'application du cloud. La chaîne est complète.

#### 5.4.5 Interpréter le résultat

| Ce que vous observez | Ce que cela signifie | Quoi faire |
| :-- | :-- | :-- |
| Les trois messages s'affichent | La chaîne complète est opérationnelle | Passez au chapitre 6 |
| La compilation échoue | Problème local, poste ou modèle | Reprenez 5.4.2 |
| La compilation réussit, le déploiement échoue | Problème d'environnement, de droits ou de réseau | Reprenez 5.4.3 |
| Le déploiement réussit, la classe est introuvable à l'exécution | Le déploiement n'a pas abouti côté serveur, ou le nom de classe est mal saisi | Vérifiez la sortie `FinOps Cloud Runtime`, puis redéployez |
| L'adresse renvoie une erreur de droits | Le compte du navigateur n'a pas le rôle requis dans l'application | Vérifiez l'utilisateur dans `System administration` > `Users` |

**Pourquoi ce test mérite ces quelques minutes.** Sans lui, un échec survenu au chapitre 6 peut avoir dix causes réparties entre le poste, l'environnement, les droits et le code. Après lui, il n'en reste qu'une : le code que vous venez d'écrire.

#### 5.4.6 Conserver ou retirer la classe

Vous pouvez la conserver : une classe exécutable de diagnostic est utile pour vérifier rapidement l'état de la chaîne après une mise à jour de l'environnement ou une réinstallation du poste. Si vous préférez un modèle propre, supprimez-la du projet et du modèle avant le commit de la section suivante.
...
### 5.6 Checklist de validation de la phase 3
...
- [ ] La classe exécutable de validation compile sans erreur.
- [ ] Le déploiement du modèle aboutit.
- [ ] L'exécution affiche les trois messages dans l'application.
...
**Point de méthode.** La recherche de l'`Application Explorer` s'appuie sur la base de références croisées construite lors de la configuration des métadonnées en 3.9. Si la recherche ne retourne rien alors que la table existe, c'est le signe que cette base n'a pas été correctement générée. Reportez-vous à l'annexe A.
...
**Étape 1.** Dans Visual Studio, ouvrez `Tools` (Outils) > `Download Dynamics 365 FnO NuGets for CI/CD` (Télécharger les NuGets Dynamics 365 FnO pour CI/CD). Cette commande est disponible car l'option correspondante a été activée en 3.6.
...
| Les générations sont anormalement lentes, sans erreur | Analyse antivirus des dizaines de milliers de fichiers de métadonnées | Posez les exclusions décrites en 3.2.4 et 3.10.1 |
| Erreur de compilation mentionnant un chemin, sans autre indication | Limite de longueur de chemin de Windows | Activez la prise en charge des chemins longs, voir 3.2.2, et raccourcissez les dossiers |
| La configuration des métadonnées refuse de s'enregistrer | Instance LocalDB absente ou arrêtée | Voir 3.10.2 |
| Aucune progression visible lors d'une opération longue | Mauvaise source dans la fenêtre de sortie | Sélectionnez `FinOps Cloud Runtime`, voir 3.10.3 |
| La classe exécutable ne s'exécute pas depuis le navigateur | Modèle non déployé, ou nom de classe erroné dans l'adresse | Vérifiez la sortie de déploiement, puis reprenez 5.4.4 |
...
| L'authentification échoue en boucle | Blocage lié à l'authentification multifacteur, `Security defaults` actifs par défaut | Décochez **toutes** les cases de l'écran de connexion pour déclencher le flux navigateur. Voir l'avertissement de la section 3.8 |
| Message `AADSTS50076` avec `suberror: basic_action` sur la ressource `00000007-0000-0000-c000-000000000000` | L'extension Power Platform Tools a tenté d'obtenir le jeton **Dataverse** par un flux hérité, incapable de présenter un second facteur | Décochez **toutes** les cases de l'écran de connexion de l'extension, puis relancez. Voir le décryptage complet en 3.8 |
...
| Seule la solution `Default` est proposée à la connexion | Aucune solution créée, ou solution créée dans un autre environnement | Vérifiez le sélecteur d'environnement sur `make.powerapps.com` et recréez la solution selon la section 3.7 |
...
| L'`Application Explorer` ne trouve pas `CustTable` | Base de références croisées non générée | Reprenez la section 3.9, et régénérez si nécessaire |
...
| Fait | Action | Rôle requis | Section |
| :-- | :-- | :-- | :-- |
| [ ] | Activer la prise en charge des chemins longs, puis redémarrer | Administrateur local | 3.2.2 |
| [ ] | Créer l'arborescence `C:\D365` | Administrateur local | 3.2.3 |
| [ ] | **Poser les exclusions antivirus sur les dossiers de développement** | Administrateur local | 3.2.4 |
| [ ] | Vérifier les accès réseau depuis le navigateur du poste | Aucun | 3.2.5 |
| [ ] | Télécharger Visual Studio 2026 Professional via Dev Essentials | Aucun | 3.3 |
| [ ] | Installer la charge de travail `.NET desktop development` | Administrateur local | 3.4 |
| [ ] | Ajouter le composant `Modeling SDK` | Administrateur local | 3.4 |
| [ ] | Ajouter le composant `DGML editor` | Administrateur local | 3.4 |
| [ ] | Vérifier la présence de `SQL Server Express LocalDB` | Administrateur local | 3.4 |
| [ ] | Se connecter à Visual Studio avec le compte du tenant | Aucun | 3.4 |
| [ ] | Installer `Microsoft Reporting Services Projects` | Administrateur local | 3.5.1 |
| [ ] | Installer `Power Platform Tools` | Administrateur local | 3.5.2 |
| [ ] | Régler les dix options `Power Platform Tools` selon le tableau | Aucun | 3.6.1 |
| [ ] | **Vérifier `Enable auto setup` et `Download logs for operations`** | Aucun | 3.6.1 |
| [ ] | Laisser `Enable Download Dynamics 365 FinOps VHD` inactive | Aucun | 3.6.1 |
| [ ] | Ouvrir `make.powerapps.com` et **vérifier le sélecteur d'environnement** | Aucun | 3.7.2 |
| [ ] | Créer l'éditeur avec un préfixe maîtrisé | `System customizer` | 3.7.2 |
| [ ] | Créer la solution rattachée à cet éditeur | `System customizer` | 3.7.3 |
| [ ] | **Vérifier le nom technique, non modifiable ensuite** | `System customizer` | 3.7.3 |
| [ ] | Cocher `Set as your preferred solution` | `System customizer` | 3.7.3 |
| [ ] | **En cas de MFA, décocher toutes les cases de l'écran de connexion** | `System administrator` | 3.8 |
| [ ] | **Se connecter avec l'`Environment URL`, et non l'adresse de l'application** | `System administrator` | 3.8 |
| [ ] | **Sélectionner votre solution, et non `Default`** | `System administrator` | 3.8 |
| [ ] | Télécharger les métadonnées de référence | `System administrator` | 3.8 |
| [ ] | Installer l'extension Finance and Operations | Administrateur local | 3.8 |
| [ ] | Vérifier les 24 Go du dossier des assets | Aucun | 3.8 |
| [ ] | Vérifier l'ouverture de l'`Application Explorer` | Aucun | 3.9 |
| [ ] | Compléter les exclusions antivirus avec les chemins effectifs | Administrateur local | 3.10.1 |
| [ ] | Vérifier l'instance LocalDB `MSSQLLocalDB` | Administrateur local | 3.10.2 |
| [ ] | Sélectionner la source de sortie `FinOps Cloud Runtime` | Aucun | 3.10.3 |
| [ ] | **Dérouler les dix contrôles de bon fonctionnement du poste** | Administrateur local | 3.11 |
...
| [ ] | Créer la classe exécutable `ArcHelloWorld` | Administrateur local | 5.4.1 |
| [ ] | Compiler le projet sans erreur | Administrateur local | 5.4.2 |
| [ ] | Déployer le modèle sur l'environnement | `System administrator` | 5.4.3 |
| [ ] | **Exécuter la classe et vérifier les trois messages** | `System administrator` | 5.4.4 |
| [ ] | Réaliser et publier le premier commit | `Contributors` | 5.5 |
...
| `Security defaults` actifs sur le tenant | Ces paramètres **bloquent le flux par code d'appareil**. Le repli `-UseDeviceCode` peut donc être refusé. Passez alors par `Connect-AzAccount -AuthScope "https://api.bap.microsoft.com/"` avant l'exécution, ou par `-AccessToken`. Voir l'avertissement de la section 3.8 |
