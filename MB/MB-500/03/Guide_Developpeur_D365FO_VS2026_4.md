...
12. Annexe D : Provisionnement d'un environnement par script
13. Annexe E : Références officielles
14. À propos, contact et communauté
...
| La `Finance and Operations Provisioning App` installée **avec `DevToolsEnabled=true`** | Sans ce paramètre, Visual Studio ne peut pas se connecter. Voir la section 1.7, qui traite ce point en détail |
...
**Contrôle 2, décisif.** Tentez la connexion depuis Visual Studio, décrite en 2.6. Si l'environnement apparaît dans la liste et propose des solutions, les outils de développement sont actifs. S'il n'apparaît pas, ou si aucune solution n'est proposée, c'est le premier soupçon.
...
#### 2.4.1 Tableau de configuration recommandée
...
#### 2.4.2 Lecture d'ensemble de ces réglages
...
| :-- | :-- |
| Interface en ligne de commande `pac`, section 6.2 | `pac auth create` ouvre le navigateur de lui-même, aucune adaptation n'est nécessaire |
| Connexion de service Azure DevOps, section 6.3 | Préférez `Workload Identity Federation` (Fédération d'identité de charge de travail) au secret client : elle supprime à la fois le secret et sa date d'expiration |
...
- [ ] Les dix options `Power Platform Tools` sont réglées conformément au tableau 2.4.1.
...
#### 5.3.1 Ce qu'est un type de données étendu et pourquoi il est obligatoire ici
...
#### 5.3.3 Renseigner les propriétés
...
### 5.4 Étape 3 : l'extension de table et la création du champ
...
#### 5.4.1 Ce qu'est une extension de table
...
| Modifier le code d'une méthode standard | Non, il faut passer par la Chain of Command de la section 5.6 |
...
#### 5.4.2 Localiser la table client dans l'Application Explorer
...
**Point de méthode.** La recherche de l'`Application Explorer` s'appuie sur la base de références croisées construite lors de la configuration des métadonnées en 2.7. Si la recherche ne retourne rien alors que la table existe, c'est le signe que cette base n'a pas été correctement générée. Reportez-vous à l'annexe A.
...
#### 5.4.3 Créer l'extension de table
...
#### 5.4.5 Renseigner les propriétés du champ
...
| `Extended Data Type` (Type de données étendu) | `ArcCreditReviewDate` | L'EDT créé en 5.3. C'est de lui que le champ hérite libellé, aide et format. |
...
#### 5.4.6 Ajouter le champ à un groupe de champs
...
2. **Poser directement le contrôle sur le formulaire** en 5.5, sans passer par un groupe. Le résultat visible pour l'utilisateur est identique.
...
#### 5.4.7 Ce qui a été produit sur le disque
...
#### 5.4.8 Ce qui se passe lors de la synchronisation de la base
...
La synchronisation, déclenchée en 5.7, exécute une commande d'ajout de colonne sur la table `CUSTTABLE` de la base de données.
...
**Conséquence directe sur votre code X++.** La date nulle vaut `false` lorsqu'elle est évaluée comme un booléen. C'est précisément ce qui rend correcte l'écriture utilisée en 5.6 :
...
#### 5.4.9 Pièges fréquents à ce stade
...
#### 5.4.10 Vérifier avant de poursuivre
...
| L'authentification échoue en boucle | Blocage lié à l'authentification multifacteur, `Security defaults` actifs par défaut | Décochez **toutes** les cases de l'écran de connexion pour déclencher le flux navigateur. Voir l'avertissement de la section 2.6 |
| Message `AADSTS50076` avec `suberror: basic_action` sur la ressource `00000007-0000-0000-c000-000000000000` | L'extension Power Platform Tools a tenté d'obtenir le jeton **Dataverse** par un flux hérité, incapable de présenter un second facteur | Décochez **toutes** les cases de l'écran de connexion de l'extension, puis relancez. Voir le décryptage complet en 2.6 |
...
| L'`Application Explorer` ne trouve pas `CustTable` | Base de références croisées non générée | Reprenez la section 2.7, et régénérez si nécessaire |
...
| [ ] | **Vérifier que l'environnement porte `DevToolsEnabled=true`** | `Power Platform admin` | 1.7.4 |
| [ ] | À défaut, recréer l'environnement par le portail ou par le script | `Power Platform admin` | 12 |
...
| [ ] | Régler les dix options `Power Platform Tools` selon le tableau | Aucun | 2.4.1 |
| [ ] | **Vérifier `Enable auto setup` et `Download logs for operations`** | Aucun | 2.4.1 |
| [ ] | Laisser `Enable Download Dynamics 365 FinOps VHD` inactive | Aucun | 2.4.1 |
...
| [ ] | **En cas de MFA, décocher toutes les cases de l'écran de connexion** | `System administrator` | 2.6 |
...
| [ ] | Créer l'EDT de type `Date`, le type primitif étant définitif | Administrateur local | 5.3.2 |
| [ ] | Renseigner `Extends`, le libellé et le texte d'aide | Administrateur local | 5.3.3 |
| [ ] | Vérifier l'EDT dans l'`Application Explorer` | Administrateur local | 5.3.4 |
| [ ] | Localiser `CustTable` par la barre de recherche | Administrateur local | 5.4.2 |
| [ ] | **Inspecter la table standard pour écarter un champ redondant** | Administrateur local | 5.4.2 |
| [ ] | Créer l'extension par `Create extension in the current project` | Administrateur local | 5.4.3 |
| [ ] | Ajouter un champ de type primitif `Date` | Administrateur local | 5.4.4 |
| [ ] | Nommer le champ avec le préfixe | Administrateur local | 5.4.5 |
| [ ] | Affecter l'EDT au champ | Administrateur local | 5.4.5 |
| [ ] | **Laisser `Label` et `Help Text` vides** | Administrateur local | 5.4.5 |
| [ ] | **Laisser `Mandatory` à `No`** | Administrateur local | 5.4.5 |
| [ ] | Ajouter le champ à un groupe de champs | Administrateur local | 5.4.6 |
| [ ] | Vérifier la présence du fichier XML d'extension | Administrateur local | 5.4.7 |
...
| [ ] | **Signaler tout point de blocage à contact@archia365.fr** | Aucun | 14 |
...
### 12.1 Ce que fait le script

| Étape | Action |
| :-- | :-- |
| 1 | Obtient un jeton d'accès pour l'interface de programmation Business Application Platform, par trois voies possibles |
| 2 | Construit le corps de la requête de création d'environnement, au format JSON |
| 3 | Transmet à la `Provisioning App` la chaîne `DevToolsEnabled=true|DemoDataEnabled=true` |
| 4 | Négocie avec le service la forme exacte du corps de requête, en essayant plusieurs emplacements pour les champs de localisation |
| 5 | Affiche un récapitulatif et demande confirmation avant toute création |
| 6 | Suit l'opération de provisionnement jusqu'à son état final |
...
### 12.4 Séquence d'utilisation recommandée
...
| `Security defaults` actifs sur le tenant | Ces paramètres **bloquent le flux par code d'appareil**. Le repli `-UseDeviceCode` peut donc être refusé. Passez alors par `Connect-AzAccount -AuthScope "https://api.bap.microsoft.com/"` avant l'exécution, ou par `-AccessToken`. Voir l'avertissement de la section 2.6 |
...
### 14.2 Data & AI France Study Group
...
### 14.3 Documents liés de la même série
